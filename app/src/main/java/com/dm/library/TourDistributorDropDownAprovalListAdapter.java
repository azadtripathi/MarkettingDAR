 ...
 */
public class TourDistributorDropDownAprovalListAdapter extends BaseAdapter  {
    private ArrayList<Distributor> _Contacts;
    private ArrayList<String> mListItems;
    private ArrayList<Distributor> mListItemsId;
    ...
    public static void setSelected(String selected) {
        TourDistributorDropDownAprovalListAdapter.selected = selected;
    }
    ...
    public static void setSelectedId(String selectedId) {
        TourDistributorDropDownAprovalListAdapter.selectedId = selectedId;
    }

    public TourDistributorDropDownAprovalListAdapter(Context context, ArrayList<String> items,
                                                     TextView tv, ArrayList<Distributor> ArraycityList1) {
        preferences2=context.getSharedPreferences("MyTourDist",Context.MODE_PRIVATE);
        ...
        for(int i = 0; i< TourPlanPopupAproval.checkSelectedDist.length; i++)
        {
            if(TourPlanPopupAproval.checkSelectedDist[i])
            {
                selectedCount++;
            }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try{
            if(TourPlanPopupAproval.checkSelectedDist[position])
            {
    ...
    /*
     * Function which updates the selected values display and information(checkSelectedDist[])
     * */
    public void setText(int position1){
        if (!TourPlanPopupAproval.checkSelectedDist[position1]) {
            TourPlanPopupAproval.checkSelectedDist[position1] = true;
            selectedCount++;
        } else {
            TourPlanPopupAproval.checkSelectedDist[position1] = false;
            selectedCount--;
        }
        ...
        if (selectedCount == 0) {
            ...
            for (int i = 0; i < TourPlanPopupAproval.checkSelectedDist.length; i++) {
                if (TourPlanPopupAproval.checkSelectedDist[i] == true) {
                    firstSelected = mListItems.get(i);
                    if(mListItemsId.get(i).getDistributor_name().equals(mListItems.get(i)))
                    {
                        firstSelectedId = mListItemsId.get(i).getDistributor_id();
...
//					firstSelected = mListItems.get(i);
                    ...
                    if(mListItemsId.get(i).getDistributor_name().equals(mListItems.get(i)))
                    {
                        firstSelectedId=firstSelectedId+mListItemsId.get(i).getDistributor_id()+",";
