...
public class CityDropDownListAdapter  extends BaseAdapter implements Filterable{
	...
	public static void setSelected(String selected) {
		CityDropDownListAdapter.selected = selected;
	}
	...
	public static void setSelectedId(String selectedId) {
		CityDropDownListAdapter.selectedId = selectedId;
	}

	public CityDropDownListAdapter(Context context, ArrayList<String> items,
									  TextView tv,ArrayList<City> ArraycityList1) {
		preferences2=context.getSharedPreferences("MyExpCity",Context.MODE_PRIVATE);
		...
		_context=context;
		for(int i = 0; i< AdvanceExpenseRequest.checkSelected.length; i++)
		{
			if(AdvanceExpenseRequest.checkSelected[i])
			{
				selectedCount++;
			}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try{
			if(AdvanceExpenseRequest.checkSelected[position])
			{
	...
	public void setText(int position1){
		if (!AdvanceExpenseRequest.checkSelected[position1]) {
			AdvanceExpenseRequest.checkSelected[position1] = true;
			selectedCount++;
		} else {
			AdvanceExpenseRequest.checkSelected[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < AdvanceExpenseRequest.checkSelected.length; i++) {
				if (AdvanceExpenseRequest.checkSelected[i] == true) {
...
//	         CityDropDownListAdapter  adapter = new CityDropDownListAdapter(_context, mListItems, mSelectedItems,_Contacts);
